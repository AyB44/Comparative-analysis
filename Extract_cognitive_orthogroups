...
# Compile a regular expression to match GO term patterns
GO_RE = re.compile(r"GO:\d{7}")
# Load a custom GO term-to-label mapping from a file, or use the default if none is provided
def load_go_map(go_file: Path | None) -> dict[str, str]:
    ...
    return mapping
# Main function to extract genes associated with cognitive GO terms from EggNOG annotations
def main():
...
# Check if the annotations file exists before proceeding
   ...
   # Load the GO term mapping and prepare data structures for storing results
  ...
  # Read the annotations file and extract relevant genes based on GO terms
    with args.annotations.open("r", encoding="utf-8", newline="") as fh:
        ...
        for row in reader:
            ...
            for go in hits:
                per_go[go].add(gene)
# Write gene lists for each GO term to separate files
...
# Write a summary file mapping each gene to its matched GO terms and labels
